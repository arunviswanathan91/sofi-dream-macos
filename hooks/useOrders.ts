...
export function useOrders() {
  ...
  useEffect(() => {
    if (!isFirebaseConfigured) {
      // Use AsyncStorage when Firebase is not set up
      getLocalOrders()
        .then(setOrders)
    ...
    const unsubscribe = onSnapshot(
      ...
      (err) => {
        // Firebase failed — fall back to local store
  ...
  return { orders, loading, addOrder, updateOrder, deleteOrder, advanceStatus, togglePaid };
}
...
async function upsertCustomer(
  ...
  try {
    ...
    // Customer upsert is non-critical — order is already saved
